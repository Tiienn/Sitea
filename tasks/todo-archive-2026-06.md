# Sitea todo archive — through June 2026

Archived from tasks/todo.md on 2026-06-12 as part of the v1 reset (specs/v1-reset-mauritius.md). Full history of completed plans and reviews below.
...
- Timed disappearance (revised per user, 2026-06-12): each print holds full opacity for 5 s and fades to nothing by 10 s, oldest first — a true alpha fade via a small ShaderMaterial (flat color + per-instance `alpha` InstancedBufferAttribute), replacing the first-pass scale-shrink. Verified in browser: fade-gradient-oldest-first.jpg shows one trail with newest prints dark and oldest dissolved; fade-recheck-11s.jpg shows clean ground after the lifetime.
...
- [x] Camera feel in CameraController + constants: FOLLOW_SMOOTH 0.1 → 0.08; right-shoulder offset 0.35 m applied to both targetCamPos and lookTarget along camera-right `(cos yaw, 0, −sin yaw)`, factor lerped 0→1 over ~0.5 s on entering TP; sprint FOV 60→66 (`currentSpeed > WALK_SPEED * 1.2`, ground speed only), damped per frame, `updateProjectionMatrix()` only when |Δfov| > 0.01, reset to 60 when walk mode disabled
- [x] View panel Avatar row (desktop panel + mobile sheet), rendered only when registry has ≥2 entries — hidden in v1; verify with a temporary dev second entry: row appears, switching rebuilds mixer cleanly, persists across reload
- [x] Remove public/models/character-casual.glb; update ATTRIBUTION.md (Mixamo character/anims on Supabase under Adobe Mixamo license; registry may host CC0 GLBs later)
- [x] Verify: eslint on changed files, npm run build, browser QA with screenshots to tasks/screenshots/realistic-avatar/ (intact worker, walk/run/jump/walkback/strafe poses, shoulder framing, FOV ease, bad-URL no-crash, footsteps/step counter/breadcrumbs/doors regression), walk the spec's acceptance criteria
- [x] Finish: spec → status built + Build notes, todo review section, push branch

### Realistic avatar Review
- Shipped: the intact Mixamo worker (helmet + vest, no mesh hiding) with its full 13-clip mocap set restored from `15a03cc^`, an avatar registry (src/constants/avatars.js, localStorage-persisted, View-panel row auto-appears at ≥2 entries), and the camera-feel package (0.35 m right-shoulder offset eased over 0.5 s, sprint FOV 60→66 with reset on leaving walk mode, FOLLOW_SMOOTH 0.08). Quaternius GLB deleted, ATTRIBUTION rewritten.
- Verified end-to-end in a real browser with trusted key events (Playwright; the embedded preview suspends rAF when backgrounded, so time-based movement can't run there — new dev helper scripts/qa-shot-server.mjs saves in-page canvas captures to tasks/screenshots/). Telemetry: walk 1.6 m/s, sprint fov 65.87→60.13, jump y 1.65→2.43→1.88, shoulder offset exactly +0.35, HUD 18 steps/13 m, 0 console errors. 13 screenshots in tasks/screenshots/realistic-avatar/.
- One pre-existing bug fixed at the root: the desktop View panel had no z-index while the Pro time slider has zIndex 10, so any panel growth (the avatar row, or just a short viewport) let the slider intercept panel clicks — panel now z-20, consistent with the mobile controls' z-30.
- Edge cases verified live: broken model URL → SilentErrorBoundary keeps app + controls alive (broken-avatar-no-crash.jpg); stale localStorage avatar id → falls back to first entry; mid-walk avatar switch rebuilds the mixer cleanly (key-based remount, cleanup in useEffect return).
- Footstep audio is code-verified only (headless can't unlock Web Audio) — worth a quick listen on a real machine.
...
- [x] Area 1 — Terrain & grass: repaint `useGrassTextures`/`useSimpleGrassTexture` with a hand-painted look (saturated lush greens, soft blotches, painterly strokes); actually apply the (currently unused) macro variation to break tiling; add a BEST-only instanced 3D grass-blade field around the plot with vertex-color gradient and gentle wind sway (plot interior excluded so blades never poke through floors)
- [x] Area 2 — Trees & vegetation: replace icosahedron canopies with stylized layered-foliage trees (2–3 variants, merged sphere clusters with vertex-color gradients, instanced per variant) plus subtle canopy wind sway; scatter instanced bushes, flowers, and rocks with the existing seeded-placement pattern (reduced counts in FAST)
- [x] Area 3 — Sky & lighting: upgrade the sky shader (domain-warped two-tone cumulus clouds with slow drift, sun glow, horizon haze); add a hemisphere light and retune `LIGHT_STOPS`/`SKY_STOPS` for warmer days and dramatic dawn/dusk; keep the `timeOfDay` 0–1 system working end-to-end
- [x] Area 4 — Distant scenery: replace billboard mountain cylinders with real displaced-ridge geometry (vertex-colored, progressively bluer with distance for aerial perspective); wire in a believable circular distant treeline (the existing `DistantTreeline` component is dead code today)
...
- Vegetation: `ScatteredTrees` rewritten as 3 layered-clump variants (broad leafy / tall cool / small bushy), each one merged vertex-colored geometry drawn as a single InstancedMesh with shared sway material; trees cast shadows in BEST. `GroundFoliage` (BEST only) adds instanced bushes, rocks, and 3 flower colors. Gotcha found: `CylinderGeometry` is indexed while `IcosahedronGeometry` is not — `mergeGeometries` returns null unless the trunk is `.toNonIndexed()`.
- Sky/lighting: sky shader v2 with domain-warped two-tone cumulus, drift, sun disc + glow tracking the real sun orbit (sunY mirrored from LIGHT_STOPS into SKY_STOPS), silver lining, horizon haze. New hemisphere light carries part of the ambient share; LIGHT_STOPS retuned (warm noon, golden dawn/dusk, blue night) with fog tracking the sky horizon. `timeOfDay` slider + auto-cycle verified end-to-end at 0.2/0.5/0.72/0.95.
- Distant scenery: mountains are now real ridge-curtain geometry (jagged displaced peaks, vertex colors fading into horizon haze), three ranges with aerial perspective, opaque + unlit + tinted per time of day via `DISTANT_TINT_STOPS` (the old billboards were 40-80% transparent and washed out by fog). The dead `DistantTreeline` became a circular ring of painted canopy clusters and is wired into the scene.
- Performance: FAST mode adds no per-frame cost class — same instanced-mesh count profile (3 tree meshes vs 2 before, +1 treeline ring), no grass blades, no foliage, no shadows; heavy work (3.5k grass instances, foliage, tree shadows) is BEST-only. All scenery is hidden in 2D mode (verified — CAD view unchanged).
- Dev QA helper: `?qaTime` URL flag (dev builds only, `import.meta.env.DEV`) unlocks the Pro time slider so visual QA can drive `timeOfDay` without a subscription.
- Verification: Playwright screenshots after every area at multiple times of day (before/after kept in `tasks/screenshots/open-world/`), 0 console errors, ESLint clean on changed files (warnings are the pre-existing `Math.random`-in-`useMemo` purity pattern), `npm run build` passes.
- Not done / future: grass blades fade-out at orbit distance (they read as fine speckle from high up — acceptable but could LOD), bloom/tone-mapping skipped (kept the dependency surface unchanged), comparison objects untouched by design.
