    ...
    <div className="w-full h-full relative">
      ...
      <Suspense fallback={<LoadingFallback />}>
        <LandScene
          ...
          timeSliderMobileTop={
            viewMode === 'firstPerson' && cameraMode === CAMERA_MODE.THIRD_PERSON
              ? 282  // controls + toggle + slider + minimap + gaps
              : viewMode === 'firstPerson'
              ? 240  // controls + toggle + minimap + gaps
              : 222  // controls + minimap + gaps
          }
      ...
      {viewMode !== '2d' && !isGuidedMode && !isMobileChatFocused && (
        <Minimap
          ...
          mobileTopOffset={
            viewMode === 'firstPerson' && cameraMode === CAMERA_MODE.THIRD_PERSON
              ? 168  // camera row + 1st/3rd toggle + slider + gaps
              : viewMode === 'firstPerson'
              ? 126  // camera row + 1st/3rd toggle + gaps
              : 108  // camera row + gap
          }
        />
      )}
        ...
        <div className={`sitea-mobile-view-controls absolute right-3 z-30 flex flex-col items-end gap-2 animate-fade-in ${isReadOnly ? 'top-20' : 'top-12'}`}>
          <div className="flex items-center gap-2">
            <div className="sitea-control-panel sitea-segment">
              {[['firstPerson', 'Walk'], ['orbit', '3D'], ['2d', '2D']].map(([mode, label]) => (
              ...
              {cameraMode === CAMERA_MODE.THIRD_PERSON && (
                <div className="sitea-control-panel rounded-xl flex items-center min-w-0" style={{ padding: '10px 12px' }}>
                  <input
                    ...
                    className="w-full h-5 accent-[var(--color-accent)] min-w-0"
                  />
                </div>
              )}
            </div>
                ...
                <>
                  ...
                  {helpGuideSection === 'views' && (
                    <div className="space-y-1.5 text-gray-300">
                      <div className="flex gap-3"><span className="text-white font-medium w-8 shrink-0">Walk</span><span>Walk your land — feel the real scale at true walking speed</span></div>
