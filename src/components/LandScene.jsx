...
function Scene({ length, width, isExploring, comparisonObjects = [], polygonPoints, placedBuildings = [], selectedBuilding, selectedBuildingType, onPlaceBuilding, onDeleteBuilding, onMoveBuilding, selectedPlacedBuildingId = null, setSelectedPlacedBuildingId, joystickInput, lengthUnit = 'm', onCameraUpdate, buildingRotation = 0, snapInfo, onPointerMove, setbacksEnabled = false, setbackDistanceM = 0, placementValid = true, overlappingBuildingIds = new Set(), labels = {}, canEdit = true, analyticsMode = 'example', cameraMode, setCameraMode, followDistance, setFollowDistance, orbitEnabled, setOrbitEnabled, viewMode = 'firstPerson', fitToLandTrigger = 0, quality = QUALITY.BEST, comparisonPositions = {}, onComparisonPositionChange, comparisonRotations = {}, onComparisonRotationChange, gridSnapEnabled = false, gridSize = 1, walls = [], wallDrawingMode = false, setWallDrawingMode, wallDrawingPoints = [], setWallDrawingPoints, addWallFromPoints, openingPlacementMode = 'none', setOpeningPlacementMode, addOpeningToWall, updateOpeningPosition, activeBuildTool = 'none', setActiveBuildTool, selectedElement, setSelectedElement, BUILD_TOOLS = {}, deleteWall, doorWidth = 0.9, doorHeight = 2.1, doorType = 'single', windowWidth = 1.2, windowHeight = 1.2, windowSillHeight = 0.9, halfWallHeight = 1.2, fenceType = 'picket', rooms = [], floorPlanImage = null, floorPlanSettings = {}, buildings = [], floorPlanPlacementMode = false, pendingFloorPlan = null, buildingPreviewPosition = { x: 0, z: 0 }, setBuildingPreviewPosition, buildingPreviewRotation = 0, placeFloorPlanBuilding, selectedBuildingId = null, setSelectedBuildingId, moveSelectedBuilding, selectedComparisonId = null, setSelectedComparisonId, selectedRoomId = null, setSelectedRoomId, roomLabels = {}, roomStyles = {}, setRoomLabel, moveRoom, moveWallsByIds, rotateWallsByIds, commitWallsToHistory, setRoomPropertiesOpen, setWallPropertiesOpen, setFencePropertiesOpen, pools = [], addPool, deletePool, updatePool, poolPolygonPoints = [], setPoolPolygonPoints, poolDepth = 1.5, selectedPoolId = null, setSelectedPoolId, setPoolPropertiesOpen, foundations = [], addFoundation, deleteFoundation, updateFoundation, foundationPolygonPoints = [], setFoundationPolygonPoints, roomPolygonPoints = [], setRoomPolygonPoints, foundationHeight = 0.6, selectedFoundationId = null, setSelectedFoundationId, setFoundationPropertiesOpen, stairs = [], addStairs, deleteStairs, updateStairs, stairsStartPoint = null, setStairsStartPoint, stairsWidth = 1.0, stairsTopY = 2.7, stairsStyle = 'straight', rotateDegreeInput = '', setRotateDegreeInput, selectedStairsId = null, setSelectedStairsId, setStairsPropertiesOpen, furnitureItems = [], addFurniture, deleteFurniture, updateFurniture, selectedFurnitureId = null, setSelectedFurnitureId, selectedFurnitureCatalogId = null, onNPCInteract, wrapperActiveNPC, currentFloor = 0, floorHeight = 2.7, totalFloors = 1, addFloorsToRoom, mobileRunning = false, mobileJumpTrigger = 0, onNearbyNPCChange, onNearbyBuildingChange, timeOfDay = 0.35, setTimeOfDay, isPaidUser = false, initialCameraPosition = null }) {
  ...
  const [rotationState, setRotationState] = useState({
    isRotating: false,
    elementType: null,   // 'room' | 'pool' | 'foundation' | 'stairs'
    elementId: null,
    center: null,        // {x, z} rotation pivot
    startAngle: null,    // initial mouse angle from center (radians)
    currentDegrees: 0,   // current rotation angle in degrees (for display)
    wallIds: [],         // for rooms
    originalWalls: [],   // snapshot for rooms (array of {id, start, end})
    originalPoints: [],  // snapshot for pools/foundations
    originalStairs: null // snapshot for stairs
  })
  const [rotationTypedDegrees, setRotationTypedDegrees] = useState('')
  ...
  const rotatePoint = (point, center, angle) => ({
    x: center.x + (point.x - center.x) * Math.cos(angle) - (point.z - center.z) * Math.sin(angle),
    z: center.z + (point.x - center.x) * Math.sin(angle) + (point.z - center.z) * Math.cos(angle)
  })

  const startRotation = (elementType, elementId, clickPoint) => {
    if (rotationState.isRotating) return // Already rotating

    let center, wallIds = [], originalWalls = [], originalPoints = [], originalStairs = null

    if (elementType === 'room') {
      const room = rooms.find(r => r.id === elementId)
      if (!room) return

      // Find all connected rooms (share a wall) to rotate the whole house together
      const allWallSets = {}
      for (const r of rooms) {
        allWallSets[r.id] = findWallsForRoom(r, walls)
      }
      const visited = new Set([elementId])
      const queue = [elementId]
      while (queue.length > 0) {
        const rid = queue.shift()
        const rWalls = allWallSets[rid] || []
        for (const otherRoom of rooms) {
          if (visited.has(otherRoom.id)) continue
          const otherWalls = allWallSets[otherRoom.id] || []
          if (rWalls.some(wid => otherWalls.includes(wid))) {
      ...
      // Collect all wall IDs from connected rooms
      const wallIdSet = new Set()
      for (const rid of visited) {
        for (const wid of (allWallSets[rid] || [])) wallIdSet.add(wid)
      }
      wallIds = [...wallIdSet]

      // Center from all connected rooms' points
      const allPts = rooms.filter(r => visited.has(r.id)).flatMap(r => r.points)
      center = {
        x: allPts.reduce((s, p) => s + p.x, 0) / allPts.length,
        z: allPts.reduce((s, p) => s + p.z, 0) / allPts.length
      }
      originalWalls = walls.filter(w => wallIds.includes(w.id)).map(w => ({
        id: w.id, start: { ...w.start }, end: { ...w.end }
      }))
    } else if (elementType === 'pool') {
      const pool = pools.find(p => p.id === elementId)
      if (!pool) return
      originalPoints = pool.points.map(p => ({ ...p }))
      center = { ...pool.center }
    } else if (elementType === 'foundation') {
      const foundation = foundations.find(f => f.id === elementId)
      if (!foundation) return
      originalPoints = foundation.points.map(p => ({ ...p }))
      ...
      originalStairs = {
        ...
        ...(stair.mid ? { mid: { ...stair.mid } } : {}),
        ...(stair.mid2 ? { mid2: { ...stair.mid2 } } : {})
      }
      // Center is midpoint of start and end
      center = {
        x: (stair.start.x + stair.end.x) / 2,
        z: (stair.start.z + stair.end.z) / 2
      }
    } else if (elementType === 'furniture') {
      const fItem = furnitureItems.find(f => f.id === elementId)
      if (!fItem) return
      // For furniture, just rotate by 90 degrees (or typed degrees) and return
      const angleDeg = rotateDegreeInput && !isNaN(parseFloat(rotateDegreeInput))
        ? parseFloat(rotateDegreeInput)
        : 90
      updateFurniture?.(elementId, { rotation: (fItem.rotation || 0) + angleDeg * Math.PI / 180 })
      if (rotateDegreeInput) setRotateDegreeInput('')
      return
    }

    // If user typed an exact degree, apply immediately and commit
    const typedDegrees = parseFloat(rotateDegreeInput)
    if (rotateDegreeInput && !isNaN(typedDegrees)) {
      const angle = typedDegrees * Math.PI / 180

      if (elementType === 'room') {
        rotateWallsByIds?.(wallIds, center, angle, originalWalls)
        commitWallsToHistory?.()
      } else if (elementType === 'pool') {
        const newPoints = originalPoints.map(p => rotatePoint(p, center, angle))
        updatePool?.(elementId, { points: newPoints })
      } else if (elementType === 'foundation') {
        const newPoints = originalPoints.map(p => rotatePoint(p, center, angle))
        updateFoundation?.(elementId, { points: newPoints })
      } else if (elementType === 'stairs' && originalStairs) {
        const updates = {
          start: rotatePoint(originalStairs.start, center, angle),
          end: rotatePoint(originalStairs.end, center, angle),
        }
        if (originalStairs.mid) updates.mid = rotatePoint(originalStairs.mid, center, angle)
        if (originalStairs.mid2) updates.mid2 = rotatePoint(originalStairs.mid2, center, angle)
        updateStairs?.(elementId, updates)
      }
      setRotateDegreeInput('')
      return // Don't enter mouse-follow rotation mode
    }

    const startAngle = Math.atan2(clickPoint.z - center.z, clickPoint.x - center.x)
  ...
  const cancelRotation = () => {
    ...
    const { elementType, elementId, wallIds, originalWalls, originalPoints, originalStairs } = rotationState

    // Restore original positions
    if (elementType === 'room' && originalWalls.length > 0) {
      // Rotate by angle 0 from originals = restore to original positions
      rotateWallsByIds?.(wallIds, rotationState.center, 0, originalWalls)
    } else if (elementType === 'pool') {
      updatePool?.(elementId, { points: originalPoints, center: rotationState.center })
    } else if (elementType === 'foundation') {
      updateFoundation?.(elementId, { points: originalPoints, center: rotationState.center })
    } else if (elementType === 'stairs' && originalStairs) {
      updateStairs?.(elementId, originalStairs)
    }

    setRotationTypedDegrees('')
    setRotationState({
      isRotating: false, elementType: null, elementId: null, center: null,
      startAngle: null, currentDegrees: 0, wallIds: [], originalWalls: [], originalPoints: [], originalStairs: null
    })
  }

  const commitRotation = () => {
    if (!rotationState.isRotating) return
    // If user typed exact degrees, snap to that angle before committing
    const typed = parseFloat(rotationTypedDegrees)
    if (rotationTypedDegrees && !isNaN(typed)) {
      const angle = typed * Math.PI / 180
      const { elementType, elementId, wallIds, originalWalls, originalPoints, originalStairs, center } = rotationState
      if (elementType === 'room') {
        rotateWallsByIds?.(wallIds, center, angle, originalWalls)
      } else if (elementType === 'pool') {
        const newPoints = originalPoints.map(p => rotatePoint(p, center, angle))
    ...
    if (rotationState.elementType === 'room') {
      commitWallsToHistory?.()
    }
    setRotationTypedDegrees('')
    setRotationState({
  ...
  const [furnitureDragState, setFurnitureDragState] = useState({ isDragging: false })
  ...
  useEffect(() => {
    if (orbitControlsRef.current) {
      const shouldBeEnabled = (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS) && !roomDragState.isDragging && !poolDragState.isDragging && !foundationDragState.isDragging && !stairsDragState.isDragging && !furnitureDragState.isDragging && !selectedBuildingId && !floorPlanPlacementMode && !selectedComparisonId && !selectedRoomId && !selectedPlacedBuildingId && !draggingOpening
      orbitControlsRef.current.enabled = shouldBeEnabled
    }
  }, [activeBuildTool, roomDragState.isDragging, poolDragState.isDragging, foundationDragState.isDragging, stairsDragState.isDragging, furnitureDragState.isDragging, selectedBuildingId, floorPlanPlacementMode, selectedComparisonId, selectedRoomId, selectedPlacedBuildingId, draggingOpening, BUILD_TOOLS])
  ...
  const handleRoomPointerDown = (e) => {
    ...
    if (!roomDragState.isDragging) {
      ...
      let currentPoint = point // Use clicked point as final point
      // Shift constrains to square
      if (shiftHeld && startPoint) {
        const dx = currentPoint.x - startPoint.x
        const dz = currentPoint.z - startPoint.z
        const side = Math.max(Math.abs(dx), Math.abs(dz))
        currentPoint = { x: startPoint.x + side * Math.sign(dx), z: startPoint.z + side * Math.sign(dz) }
      }
  ...
  const handleLandClick = (e) => {
    ...
    // Rotation tool - commit rotation on second click
    ...
    // Skip this check when a build/placement tool is active (clicks forwarded from walls/floors)
    const isPlacementTool = isWallMode || activeBuildTool === BUILD_TOOLS.FURNITURE || activeBuildTool === BUILD_TOOLS.POOL || activeBuildTool === BUILD_TOOLS.FOUNDATION || activeBuildTool === BUILD_TOOLS.STAIRS
    if (!isPlacementTool && e.intersections && e.intersections.length > 1) {
    ...
    // Wall drawing mode — handle before deselection so clicks on walls/floors work
    ...
    // Deselect furniture when clicking on empty land
    ...
    // Furniture tool - single-click placement
    if (activeBuildTool === BUILD_TOOLS.FURNITURE && selectedFurnitureCatalogId) {
      e.stopPropagation()
      const snapped = snapRoomPoint(e.point)
      addFurniture?.(selectedFurnitureCatalogId, snapped)
      return
    }
  ...
  const handlePointerMove = (e) => {
    ...
    // Rotation tool - update rotation preview
    if (rotationState.isRotating) {
      const { center, startAngle, elementType, elementId, wallIds, originalWalls, originalPoints, originalStairs } = rotationState

      // If user typed exact degrees, use that instead of mouse angle
      const typedVal = parseFloat(rotationTypedDegrees)
      let deltaAngle
      if (rotationTypedDegrees && !isNaN(typedVal)) {
        deltaAngle = typedVal * Math.PI / 180
      } else {
        let currentAngle = Math.atan2(point.z - center.z, point.x - center.x)
        deltaAngle = currentAngle - startAngle
        // Shift snap to 45 degrees
        if (shiftHeld) {
          deltaAngle = Math.round(deltaAngle / (Math.PI / 4)) * (Math.PI / 4)
        }
      }

      // Track degrees for display
      let degrees = ((deltaAngle * 180 / Math.PI) % 360)
      if (degrees > 180) degrees -= 360
      if (degrees < -180) degrees += 360
      setRotationState(prev => ({ ...prev, currentDegrees: Math.round(degrees) }))

      if (elementType === 'room') {
        rotateWallsByIds?.(wallIds, center, deltaAngle, originalWalls)
      } else if (elementType === 'pool') {
        const newPoints = originalPoints.map(p => rotatePoint(p, center, deltaAngle))
        updatePool?.(elementId, { points: newPoints })
      } else if (elementType === 'foundation') {
        const newPoints = originalPoints.map(p => rotatePoint(p, center, deltaAngle))
        updateFoundation?.(elementId, { points: newPoints })
        ...
        const updates = {
          start: rotatePoint(originalStairs.start, center, deltaAngle),
          end: rotatePoint(originalStairs.end, center, deltaAngle),
        }
        if (originalStairs.mid) updates.mid = rotatePoint(originalStairs.mid, center, deltaAngle)
        if (originalStairs.mid2) updates.mid2 = rotatePoint(originalStairs.mid2, center, deltaAngle)
        updateStairs?.(elementId, updates)
      }
      return
    }
    ...
    // Opening drag - slide opening along its wall in 2D mode
    if (draggingOpening && updateOpeningPosition) {
      const wall = walls.find(w => w.id === draggingOpening.wallId)
      if (wall) {
        ...
        if (wallLen > 0.01) {
          ...
          const t = Math.max(0, Math.min(1,
            ((point.x - wall.start.x) * dx + (point.z - wall.start.z) * dz) / (wallLen * wallLen)
          ))
          const posOnWall = t * wallLen
          // Validate: skip self when checking overlaps
          const otherOpenings = (wall.openings || []).filter((_, i) => i !== draggingOpening.openingIndex)
          const validation = isValidOpeningPlacement(wall, posOnWall, draggingOpening.openingWidth, otherOpenings)
          if (validation.valid) {
            updateOpeningPosition(draggingOpening.wallId, draggingOpening.openingIndex, posOnWall)
          }
        }
      }
      return
    ...
    if (roomDragState.isDragging && activeBuildTool === BUILD_TOOLS.ROOM) {
      ...
      // Shift constrains to square
      if (shiftHeld && roomDragState.startPoint) {
        const dx = snappedPoint.x - roomDragState.startPoint.x
        const dz = snappedPoint.z - roomDragState.startPoint.z
        const side = Math.max(Math.abs(dx), Math.abs(dz))
        snappedPoint = { x: roomDragState.startPoint.x + side * Math.sign(dx), z: roomDragState.startPoint.z + side * Math.sign(dz) }
      }
  ...
  // Escape key to cancel rotate tool / active rotation + digit input during rotation
  useEffect(() => {
    if (activeBuildTool !== BUILD_TOOLS.ROTATE) return
    ...
    const handleKeyDown = (e) => {
      ...
      // During active rotation, capture digit keys to type exact degrees
      if (rotationState.isRotating) {
        if (e.key === ' ' || e.key === 'Enter') {
          ...
          e.preventDefault()
          setRotationTypedDegrees(prev => prev + e.key)
        } else if (e.key === 'Backspace') {
          e.preventDefault()
          setRotationTypedDegrees(prev => prev.slice(0, -1))
        } else if (e.key === '-') {
          e.preventDefault()
          setRotationTypedDegrees(prev => prev.startsWith('-') ? prev.slice(1) : '-' + prev)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (e) => {
      ...
      if (activeBuildTool === BUILD_TOOLS.POLYGON_ROOM) {
        ...
        setWallDrawingPoints?.([])
        setWallDrawingMode?.(false)
      } else if (activeBuildTool === BUILD_TOOLS.ROTATE && rotationState.isRotating) {
        cancelRotation()
      }
      setActiveBuildTool?.(BUILD_TOOLS.NONE)
    }
  ...
  }, [activeBuildTool, BUILD_TOOLS, setActiveBuildTool, setWallDrawingPoints, setWallDrawingMode, rotationState.isRotating])
  ...
  // Reset camera init flag when guided mode activates (initialCameraPosition goes from null to a value)
  useEffect(() => {
  ...
  }, [length, width, polygonPoints, camera, initialCameraPosition])
    ...
    <>
      ...
      {pools.map((pool) => (
        <PoolItem
          ...
          isDeleteMode={activeBuildTool === BUILD_TOOLS.DELETE}
          isRotateMode={activeBuildTool === BUILD_TOOLS.ROTATE && !rotationState.isRotating}
          onDelete={deletePool}
          onUpdate={updatePool}
      ...
      {furnitureItems.map((item) => {
        const catalogEntry = FURNITURE_ITEMS.find(f => f.id === item.catalogId)
        ...
        return (
          <FurnitureItem
            ...
            onRotateStart={startRotation}
            onDragStart={() => setFurnitureDragState({ isDragging: true })}
            onDragEnd={() => setFurnitureDragState({ isDragging: false })}
            snapPoint={snapRoomPoint}
          />
        )
      })}

      {/* Rotation angle indicator */}
      {rotationState.isRotating && rotationState.center && (
        <PreviewDimensionLabel
          position={[rotationState.center.x, 1.5, rotationState.center.z]}
          text={rotationTypedDegrees ? `${rotationTypedDegrees}°` : `${rotationState.currentDegrees}°`}
          color={rotationTypedDegrees ? '#3b82f6' : '#f59e0b'}
          fontSize={0.5}
        />
      )}
        ...
        return (
          <RoomFloor
            ...
            onSelect={() => {
              ...
              // Don't select room when a placement tool is active
              if (activeBuildTool === BUILD_TOOLS.FURNITURE || activeBuildTool === BUILD_TOOLS.POOL || activeBuildTool === BUILD_TOOLS.FOUNDATION || activeBuildTool === BUILD_TOOLS.STAIRS) {
                return
              }
            ...
            isDrawingTool={activeBuildTool === BUILD_TOOLS.WALL || activeBuildTool === BUILD_TOOLS.HALF_WALL || activeBuildTool === BUILD_TOOLS.FENCE || activeBuildTool === BUILD_TOOLS.FURNITURE || activeBuildTool === BUILD_TOOLS.POOL || activeBuildTool === BUILD_TOOLS.FOUNDATION || activeBuildTool === BUILD_TOOLS.STAIRS}
            onDrawingClick={handleLandClick}
            isRotateMode={activeBuildTool === BUILD_TOOLS.ROTATE && !rotationState.isRotating}
            onRotateStart={startRotation}
          />
        )
      ...
      {walls.map((wall) => {
        ...
        <WallSegment
          ...
          onOpeningDragStart={viewMode === '2d' && effectiveOpeningMode === 'none' && (activeBuildTool === BUILD_TOOLS.SELECT || activeBuildTool === BUILD_TOOLS.NONE) ? (wallId, openingIndex, openingWidth) => {
            if (orbitControlsRef.current) orbitControlsRef.current.enabled = false
            setDraggingOpening({ wallId, openingIndex, openingWidth })
          } : undefined}
          isDrawingTool={activeBuildTool === BUILD_TOOLS.WALL || activeBuildTool === BUILD_TOOLS.HALF_WALL || activeBuildTool === BUILD_TOOLS.FENCE || activeBuildTool === BUILD_TOOLS.FURNITURE || activeBuildTool === BUILD_TOOLS.POOL || activeBuildTool === BUILD_TOOLS.FOUNDATION || activeBuildTool === BUILD_TOOLS.STAIRS}
          onDrawingClick={handleLandClick}
      ...
      {orbitEnabled && (
        <OrbitControls
          ...
          target={orbitTarget}
          enabled={!selectedBuildingId && !floorPlanPlacementMode && !selectedComparisonId && !roomDragState.isDragging && !poolDragState.isDragging && !foundationDragState.isDragging && !stairsDragState.isDragging && !furnitureDragState.isDragging && !selectedPlacedBuildingId && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)}
      ...
      {viewMode === '2d' && (
        <>
          ...
          <MapControls
            ...
            enabled={!roomDragState.isDragging && !poolDragState.isDragging && !foundationDragState.isDragging && !stairsDragState.isDragging && !furnitureDragState.isDragging && !draggingOpening && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)}
            enableRotate={false}
            enableDamping={false}
            enablePan={!draggingOpening && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)}
            enableZoom={!draggingOpening && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)}
            minZoom={1}
            maxZoom={100}
            maxPolarAngle={0}
            minPolarAngle={0}
            ...
            screenSpacePanning={true}
            mouseButtons={{
              LEFT: (!draggingOpening && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)) ? THREE.MOUSE.PAN : null,
              MIDDLE: (!draggingOpening && (activeBuildTool === BUILD_TOOLS.NONE || activeBuildTool === BUILD_TOOLS.ROOF || activeBuildTool === BUILD_TOOLS.ADD_FLOORS)) ? THREE.MOUSE.PAN : null,
              RIGHT: null
            }}
            touches={{
...
function TimeSlider({ timeOfDay, setTimeOfDay, mobileTop = 248 }) {
